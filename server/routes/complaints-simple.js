...
// @desc    Check for duplicate complaints before creating
// @access  Private
router.post('/check-duplicates', auth, [
  ...
  try {
    ...
    const newComplaint = { title, description, category };
    const duplicateResult = await duplicateDetectionService.checkForDuplicates(newComplaint, threshold);
...
// @route   GET /api/complaints/similar/:id
...
router.get('/similar/:id', auth, async (req, res) => {
  try {
    ...
    // Check if user can view this complaint
    if (req.user.role === 'citizen' && complaint.user.toString() !== req.userId.toString()) {
      return res.status(403).json({ message: 'Access denied' });
    }

    const threshold = parseFloat(req.query.threshold) || 0.8;
    const similarComplaints = await duplicateDetectionService.findSimilarComplaints(complaint, threshold);
    ...
    res.status(500).json({ message: 'Server error finding similar complaints' });
  }
});

// @route   POST /api/complaints/:id/enhance-priority
// @desc    Enhance priority of existing complaint when citizen reports same issue
// @access  Private
router.post('/:id/enhance-priority', auth, [
  ...
  try {
    ...
    // Enhance priority based on current priority
    ...
    // If already urgent, keep it urgent

    // Add update to history
    complaint.updates.push({
      message: `Priority enhanced from ${complaint.priority} to ${newPriority}. Reason: ${reason}`,
    ...
    io.to(`complaint-${complaint._id}`).emit('priority-enhanced', {
      complaint: await Complaint.findById(complaint._id).populate('user', 'name email')
    });

    res.json({
      message: 'Complaint priority enhanced successfully',
      complaint: await Complaint.findById(complaint._id).populate('user', 'name email')
    });
  } catch (error) {
    console.error('Enhance priority error:', error);
    res.status(500).json({ message: 'Server error enhancing priority' });
  }
});
