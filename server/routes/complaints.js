...
// @route   POST /api/complaints/demo-potholes
// @desc    Create demo pothole complaints for testing duplicate detection
// @access  Private (Admin only)
router.post('/demo-potholes', auth, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
    }

    const demoComplaints = await duplicateDetectionService.createDemoPotholeComplaints();
    ...
    res.json({
      message: 'Demo pothole complaints created successfully',
      complaints: createdComplaints
    });
  } catch (error) {
    console.error('Create demo complaints error:', error);
    res.status(500).json({ message: 'Server error creating demo complaints' });
  }
});
